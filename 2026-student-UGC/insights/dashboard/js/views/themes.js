...
export async function render() {
  ...
  const tpl = html`
    ...
    <div class="theme-list">${sorted.map(t => themeCard(t, quoteById))}</div>
  `;
  mount(root, tpl);
...
function themeCard(t, quoteById) {
  const tag = t.is_emergent ? "emergent" : "canonical";
  const samples = (t.sample_quote_ids || []).map(id => quoteById.get(id)).filter(Boolean).slice(0, 3);
