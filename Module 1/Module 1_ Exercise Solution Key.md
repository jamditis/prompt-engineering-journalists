...
For Part 5, I ran `/init` in my project folder and Gemini generated a context file based on the files that were already there — the test article and the changelog. It was mostly boilerplate, so I edited it to add my beat: "I cover the county board of commissioners for the Herald-Tribune. The board meets twice monthly and calls its budget process the 'fiscal review cycle.' I follow AP style with no Oxford comma." I quit the session and started a new one, then asked "what do I cover?" and it knew immediately. That was the moment I understood the value — I will not have to re-explain my beat every session.
