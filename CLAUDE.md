...
- **Module 3 video alignment (Apr 2026)**: All 7 Module 3 docs updated to match recorded C1/C2 videos. Videos cover systems thinking, plan mode, scheduling (remote triggers, GitHub Actions), and Mamdani pipeline continuation — not the cron scheduler or Reroute NJ walkthrough originally planned. Quiz rewritten for module concepts. Discussion simplified to 2 questions. Readings trimmed to 3 articles, optional resources to 3. Google Docs in Knight Center delivery folder populated and synced. PR #24. Full LMS bundle body rewrite to match filmed videos: PR #32.
- **Module 4 video alignment (Apr 2026)**: All 7 Module 4 LMS docs rewritten to match the filmed D1–D4 videos — `/effort` levels and context window as a budget (D1), the 40% rule, sub-agents, and rewinding (D2), cross-model code review via the `-p` flag (D3), `/remote-control` and four questions and editorial judgment (D4). Exercise replaced with a 10-step context-management and cross-model review workflow (`/compact` with and without instructions, double-escape rewind, `-p` call-out to a different model family, reconciled judgment). Framing across all Module 4 LMS files **and** the public-site Module 4 page (`docs/module-4/index.html`) and the syllabus pages was scoped to the *code side* of the workflow specifically: the skills that matter are the project-coordinator skills a non-developer uses to work with a developer on a code change, not delegating the writing or editing of journalism. Reporting, writing, editing, and editorial judgment stay with the journalist — Module 4 is about coordinating the scripts, skills, and pipelines the model helps you build, nothing else. Cross-model review `-p` language updated throughout (including in the SYLLABUS.md / Introduction Module/Syllabus.md / docs/syllabus/index.html context-isolation bullet) to clarify "cheaper output, not free output" — the other tool's reasoning runs in its own process, but the returned review text does consume tokens when it lands in the main session. Solution Key model-submission example in Module 4 is currently a journalism-draft placeholder and is flagged for a follow-up rewrite to a code/pipeline review example — Joe is handling that decision separately. PRs: #33 (LMS bundle body rewrite + framing rewrite), #34 (follow-up cleanup: Module 3 Midweek placeholder, Solution Key label fix, syllabus framing + context-isolation overstatement, public-site Module 4 page framing fix).
- **Framing rule of record**: When writing any Module 4 or management-framing content, scope the "prompting → managing" shift to the *code side* of the workflow. Never imply the model is doing the journalism writing or editing. The model writes scripts, skills, pipelines, and configs; the journalist coordinates that work the way a non-developer works with a developer. Journalism writing, editing, and judgment stay with the journalist. Language to avoid: "you are not the writer anymore," "the model is the writer," "the journalist as product manager" (unscoped), "the same skills you'd use with a new staff writer," "the model is your junior developer."
